    ...
    if (formData.email.includes("@pvkkit.ac.in") && formData.password.length > 6) {
      try {
        ...
        if (response && response.data && response.data.message === "Success") {
          ...
          setIsLoading(false);
          pageRoute("/verify");
        } else if (response && response.data && response.data.message === "User already exists") {
          toast.error(response.data.message);
          setIsLoading(false);
        } else {
          toast.error("Invalid Credentials!/User already exists");
          setIsLoading(false);
        }
      } catch (error) {
      ...
      setIsLoading(false);
      toast.warning("Provide a valid PVKKIT email address and password (at least 6 characters).");
    ...
    <div className='bg-[#121418] w-[100vw] h-[100vh] flex justify-center items-center' style={{ backgroundImage: `url(${backgroundImage})`, backgroundSize: 'cover' }}>
      <div className='w-[90%] sm:w-[400px] pl-0 ml-0 h-[400px] sm:pl-0 sm:ml-9 mt-10 relative'>
        <div className='absolute -top-7 left-0'>
          ...
          <p className='text-[#000] text-[12px] tracking-wider font-medium'>Have Account ? <Link className='text-black underline' to="/login">Sign in</Link></p>
        </div>
        <form className='flex flex-col gap-y-3 mt-[12%]' onSubmit={handleOnSubmit}>
          ...
          <div className='relative flex flex-col gap-y-3'>
            ...
            {
              !showPass ? <button type='button'><FaEyeSlash onClick={() => setShowPass(!showPass)} className='text-[#fff] absolute top-3 right-4 sm:right-6 w-[30px] h-[25px]' /></button> : <button type='button'> <FaEye onClick={() => setShowPass(!showPass)} className='text-[#fff] absolute top-3 right-4 sm:right-6 w-[30px] h-[25px]' /></button>
            }
